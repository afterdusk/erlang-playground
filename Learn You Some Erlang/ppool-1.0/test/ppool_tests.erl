...
start_test_() ->
    {"It should be possible to start a pool server and give it a name",
     {setup, fun find_unique_name/0, fun(Name) -> [start_and_test_name(Name)] end}}.
...
mfa_test_() ->
    {"A pool process can be allocated which will be ordered "
     ...
     {setup, fun start_ppool/0, fun kill_ppool/1, fun(Name) -> [pool_run_mfa(Name)] end}}.
...
realloc_test_() ->
    {"When an allocated process dies, "
     ...
     {setup, fun start_ppool/0, fun kill_ppool/1, fun(Name) -> [pool_run_realloc(Name)] end}}.
...
queue_test_() ->
    {"The queue function can be used to run the function as soon as possible. "
     ...
     {foreach,
      fun start_ppool/0,
      fun kill_ppool/1,
      [fun(Name) -> test_async_queue(Name) end, fun(Name) -> test_sync_queue(Name) end]}}.
...
supervision_test_() ->
    {"The ppool will never restart a dead child, but all children (OTP "
     ...
     "are trapping exits",
     {setup, fun find_unique_name/0, fun test_supervision/1}}.
...
start_and_test_name(Name) ->
    ...
    timer:sleep(100),
    B = whereis(Name),
    [?_assert(undefined =/= A), ?_assertEqual(undefined, B)].
...
pool_run_realloc(Name) ->
    ...
    L = flush(),
    [?_assert(is_pid(Pid)),
     ?_assertEqual([{A, i_am_running}, {B, i_am_running}, {Pid, i_am_running}], L)].
